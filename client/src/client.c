...
int sendFile(int fd, char *file_name)
{
	...
	if(send(fd, &msg, sizeof(struct msg_head), 0) != sizeof(struct msg_head))
	{
		perror("Send file name error\n");
		return -1;
	}
	if(send(fd, file_name, msg.len, 0) != msg.len)
	{
		perror("Send file name error\n");
		return -1;
	}
	if(recv(fd, &msg, sizeof(struct msg_head), 0) != sizeof(struct msg_head))
	{
		perror("Send file name error\n");
		return -1;
	}
	...
	{
		transfored_data += read_size; // jingwei ma
		msg.type = PRO_DATA;
		msg.len = read_size;
		if(send(fd, &msg, sizeof(struct msg_head), 0) != sizeof(struct msg_head))
		{
		...
		while(read_size > 0)
		{
			if((send_size = send(fd, send_buf, read_size, 0)) < 0)
			{
				perror("Send file name error\n");
				return -1;
			}
...
int backup(int fd, char * in_file, char *out_file, int trans_flag)
{
	...
	if((send_size = send(fd, &msg, sizeof(msg), 0)) != sizeof(msg))
	{
		perror("Send file name error\n");
		exit(1);
	}
	if((send_size = send(fd, out_file, msg.len, 0)) != msg.len)
	{
		perror("Send file name error\n");
		exit(1);
	}
	if((read_size = recv(fd, &msg, sizeof(struct msg_head), 0)) != sizeof(struct msg_head))
	{
	...
	/*if((send_size = send(fd, out_file, msg.len, 0)) < 0)
	{
		perror("Send file name error\n");
		exit(1);
	}*/
