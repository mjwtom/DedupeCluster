...
int create_file(char * name, struct dedupe_manager * dedup)
{
	struct file f;
	strcpy(f.name, name);
...
int end_file(struct dedupe_manager * dedup)
{
	int pos;
...
int write_file(struct file f, struct dedupe_manager * dedup, char * name)
{
	FILE * fp;
	char * buf;
#ifdef COMPRESS
...
int main2(int argc, char* argv[])
{
	...
	dedup = malloc(sizeof(struct dedupe_manager));
	dedupe_init(dedup, *argv);
	argc --;  argv ++;
	buf = malloc (READ_LEN);
	dir=opendir(*argv);
	if(NULL == dir)
...
int check_save(char * fp, char * chunk, uint32_t len, struct dedupe_manager * dm)
{
	int ret = 0;
	int need_load_cache;
	struct metadata mtdata;
	...
	need_load_cache = 0;
	mtdata.len = len;
	memcpy(mtdata.fingerprint, fp, FINGERPRINT_LEN);
	ret = bloom_filter_lookup(dm->bf, (unsigned int *) mtdata.fingerprint);
	if(1 == ret)
	{
		ret = lookup_in_cache(&dm->cache, mtdata.fingerprint, &mtdata);
		if(0 == ret)
		{
			ret = lookup_fingerprint_in_disk_hash(&dm->disk_hash, mtdata.fingerprint, &node);
			if(1 == ret)
			{
	...
	if(0 == ret)
	{
		mtdata.offset = add_data(chunk, len, &dm->dt_seg);
		node.data_len = len;
		node.mtdata_offset = add_metadata(mtdata, &(dm->mt_seg));
		node.data_offset = mtdata.offset;
		memcpy(node.fingerprint, mtdata.fingerprint, FINGERPRINT_LEN);
		add_2_disk_hash(&dm->disk_hash, node);
	}
	else
	{
		add_metadata(mtdata, &(dm->mt_seg));
		if(need_load_cache)
		{
			load_cache(&(dm->mt_seg), dm->mtdata, node.mtdata_offset, &dm->cache);
		}
	}
	return ret;
}
